...
//----------------------------------------------------------------------------
//  ec_bls509_fp : EC_fp := Y^2 = X^3 + 1 CLN
//----------------------------------------------------------------------------
//  2019.3.27 created by rclarisse
//============================================================================
...
void ec_bls509_fp_point_set_str(EC_POINT P, const char* s)
{
    ...
    bls509_fp_set_str(xcoord(P), p1);
    bls509_fp_set_str(ycoord(P), p2);
    bls509_fp_set_one(zcoord(P));
...
void ec_bls509_fp_point_get_str(char *s, const EC_POINT P)
{
    static const char infinity[] = "[0]";
    ...
    bls509_fp_get_str(sx, xcoord(T));
    bls509_fp_get_str(sy, ycoord(T));
...
void ec_bls509_fp_point_set_infinity(EC_POINT P)
{
...
//-------------------------------------------
void ec_bls509_fp_add(EC_POINT R, const EC_POINT P, const EC_POINT Q)
{
    ...
    bls509_fp_sub(t[0], xcoord(Q), xcoord(P));
    bls509_fp_sub(t[1], ycoord(Q), ycoord(P));
    bls509_fp_inv(t[0], t[0]);
    bls509_fp_mul(t[2], t[1], t[0]); // t2 = (yQ - yP)/(xQ - xP) (line coeff dir)

    bls509_fp_sqr(t[0], t[2]);
    bls509_fp_sub(t[0], t[0], xcoord(P));
    bls509_fp_sub(t[0], t[0], xcoord(Q)); // t0 = t2^2 - xP - xQ (abscisse of (P+Q))

    bls509_fp_sub(t[1], xcoord(P), t[0]);       // t1 = xP - t0
    bls509_fp_mul(t[1], t[2], t[1]);            // t1 = (xP - t0)*(yQ - yP)/(xQ - xP)
    bls509_fp_sub(ycoord(R), t[1], ycoord(P));  // yR = t1 - xP
    bls509_fp_set(xcoord(R), t[0]);             // xR = t0
    bls509_fp_set_one(zcoord(R));

    R->isinfinity = FALSE;
}

void ec_bls509_fp_dob(EC_POINT R, const EC_POINT P)
{
    Element *t = field(R)->tmp;
    ...
    bls509_fp_mul(t[2], t[1], t[0]);           // t2 = 3*xP^2 / (2*yP)

    bls509_fp_sqr(t[0], t[2]);                 // t0 = t2^2
    bls509_fp_sub(t[0], t[0], xcoord(P));
    bls509_fp_sub(t[0], t[0], xcoord(P));      // xR = (3*xP^2 / (2*yP))^2 - 2*xP

    bls509_fp_sub(t[1], xcoord(P), t[0]);      // t1 = xP - xR
    bls509_fp_mul(t[1], t[2], t[1]);           // t1 = (3*xP^2 / (2*yP)) * (xP - xR)
    bls509_fp_sub(ycoord(R), t[1], ycoord(P)); // yR = t1 - yP
    bls509_fp_set(xcoord(R), t[0]);
    bls509_fp_set_one(zcoord(R));
...
void ec_bls509_fp_neg(EC_POINT z, const EC_POINT x)
{
    ...
    bls509_fp_set(xcoord(z), xcoord(x));
    bls509_fp_neg(ycoord(z), ycoord(x));
    bls509_fp_set(zcoord(z), zcoord(x));
...
void ec_bls509_fp_add_formul(EC_POINT R, const EC_POINT P, const EC_POINT Q)
{
    Element *t = field(R)->tmp;
    ...
    bls509_fp_sqr(t[0], zcoord(P));       // A = Pz^2
    bls509_fp_mul(t[1], t[0], zcoord(P)); // B = Pz^3
    bls509_fp_mul(t[2], t[0], xcoord(Q)); // C = Qx*A
    bls509_fp_mul(t[1], t[1], ycoord(Q)); // D = Qy*B
    bls509_fp_sub(t[2], t[2], xcoord(P)); // E = C-Px
    bls509_fp_sub(t[1], t[1], ycoord(P)); // F = D-Py

    if (element_is_zero(t[2]))
    {
        if (element_is_zero(t[1]))
        {
            ec_bls509_fp_dob_formul(R, P);
            return;
        }
        point_set_infinity(R);
        return;
    ...
    bls509_fp_mul(zcoord(R), zcoord(P), t[2]); // Rz = Pz*E

    bls509_fp_sqr(t[0], t[2]);            // G = E^2
    bls509_fp_mul(t[3], xcoord(P), t[0]); // I = Px*G
    bls509_fp_mul(t[4], t[0], t[2]);      // H = G*E

    bls509_fp_sqr(xcoord(R), t[1]);
    bls509_fp_sub(xcoord(R), xcoord(R), t[4]);
    bls509_fp_add(t[0], t[3], t[3]);
    bls509_fp_sub(xcoord(R), xcoord(R), t[0]); // Rx = F^2-H-2*I

    bls509_fp_sub(t[3], t[3], xcoord(R));
    bls509_fp_mul(t[0], t[4], ycoord(P));
    bls509_fp_mul(ycoord(R), t[1], t[3]);
    bls509_fp_sub(ycoord(R), ycoord(R), t[0]); // Ry = F*(I-Rx)-Py*H

    R->isinfinity = FALSE;
}

void ec_bls509_fp_dob_formul(EC_POINT R, const EC_POINT P)
{
    Element *t = field(R)->tmp;
    ...
    bls509_fp_sqr(t[0], ycoord(P));     // A = Py^2
    bls509_fp_add(t[1], xcoord(P), xcoord(P));
    bls509_fp_add(t[1], t[1], t[1]);
    bls509_fp_mul(t[1], t[1], t[0]);    // B = 4*Px*A
    bls509_fp_sqr(t[0], t[0]);
    bls509_fp_add(t[0], t[0], t[0]);
    bls509_fp_add(t[0], t[0], t[0]);
    bls509_fp_add(t[0], t[0], t[0]);    // C = 8*A^2
    bls509_fp_sqr(t[3], xcoord(P));
    bls509_fp_add(t[2], t[3], t[3]);
    bls509_fp_add(t[2], t[2], t[3]);    // D = 3*Px^2

    bls509_fp_sqr(xcoord(R), t[2]);
    bls509_fp_add(t[3], t[1], t[1]);
    bls509_fp_sub(xcoord(R), xcoord(R), t[3]); // Rx = D^2 - 2*B

    bls509_fp_mul(zcoord(R), ycoord(P), zcoord(P));
    bls509_fp_add(zcoord(R), zcoord(R), zcoord(P)); // Rz = 2*Py*Pz

    bls509_fp_sub(ycoord(R), t[1], xcoord(R));
    bls509_fp_mul(ycoord(R), ycoord(R), t[2]);
    bls509_fp_sub(ycoord(R), ycoord(R), t[0]);  // Ry = D*(B-Rx)-C
...
//--------------------------------------------------------------
void ec_bls509_fp_mul_affine(EC_POINT Q, const mpz_t s, const EC_POINT P)
{
    long t, i;
...
//--------------------------------------------------------------
void ec_bls509_fp_mul(EC_POINT Q, const mpz_t s, const EC_POINT P)
{
    long t, i;
    EC_POINT R;
    ...
    ec_bls509_fp_point_set(R, P);

    t = mpz_sizeinbase(s, 2);

    for (i = t - 2; i >= 0; i--)
    {
        ec_bls509_fp_dob_formul(R, R);
        if (mpz_tstbit(s, i))
        {
            ec_bls509_fp_add_formul(R, R, P);
        }
    }
...
//------------------------------------------------------
void ec_bls509_fp_mul_naf(EC_POINT Q, const mpz_t s, const EC_POINT P)
{
    long t, i;
    ...
    for (i = nlen - 2; i >= 0; i--)
    {
        ...
        {
            if (naf[i] < 0) {
                ec_bls509_fp_add_formul(R, R, mP);
            }
            else {
                ec_bls509_fp_add_formul(R, R, P);
            }
...
//     mpz_t lambda;
//
//     d = (ec_data_fp)malloc(sizeof(struct ec_bls509_fp_ec_data_st));
//
//     if (strcmp(ec->curve_name, "ec_bls509_fpa") == 0)
//     {
...
//     }
//
//     if (strcmp(ec->curve_name, "ec_bls509_fpb") == 0)
//     {
//         element_init(d->beta, ec->field);
...
//
//     ec_bls509_fp_decompose_scalar_init(d->a1, d->a2, d->b1, d->b2, d->n, lambda);
//
//     ec->ec_data = (void*)d;
//
//     mpz_clear(lambda);
...
//     k = ( k1 + k2*l ) mod n
//----------------------------------------------------------------------------------
// void ec_bls509_fp_decompose_scalar(mpz_t k1, mpz_t k2, const mpz_t k, ec_data_fp d)
// {
//     mpz_t c1, c2, t1, t2;
//
//     mpz_init(c1);
...
//-----------------------------------------------------
// void ec_bls509_fp_mul_end(EC_POINT Q, const mpz_t s, const EC_POINT P)
// {
//     mpz_t s1, s2;
//
//     int sl1, sl2, *sn1, *sn2;
...
//     d = (ec_data_fp)(curve(P)->ec_data);
//
//     ec_bls509_fp_decompose_scalar(s1, s2, s, d);
//
...
//
//     ec_bls509_fp_point_set(P1, P);
//     ec_bls509_fp_point_endomorphism(P2, P1);
//
//     ec_bls509_fp_neg(mP1, P1);
//     ec_bls509_fp_neg(mP2, P2);
//
//     ec_bls509_fp_point_set_infinity(Q);
//
//     for (i = t - 1; i >= 0; i--)
//     {
//         ec_bls509_fp_dob_formul(Q, Q);
//
//         if (sn1[i])
//         {
//             if (sn1[i] < 0) {
//                 ec_bls509_fp_add_formul(Q, Q, mP1);
//             }
//             else {
//                 ec_bls509_fp_add_formul(Q, Q, P1);
//             }
//         }
//
//         if (sn2[i])
//         {
//             if (sn2[i] < 0) {
//                 ec_bls509_fp_add_formul(Q, Q, mP2);
//             }
//             else {
//                 ec_bls509_fp_add_formul(Q, Q, P2);
//             }
//         }
//     }
//
...
int ec_bls509_fp_cmp(const EC_POINT P, const EC_POINT Q)
{
    if (element_cmp(xcoord(P), xcoord(Q)) == 0)
    {
        if (element_cmp(ycoord(P), ycoord(Q)) == 0) {
...
// // BS2FQE for fp
// //------------------------------------------
// void bls509_fp_BS2FQE(Element z, const unsigned char *os, const size_t oslen, int t)
// {
//     size_t tlen = oslen + 2;
...
//
//         bls509_fp_BS2FQE(x0, id, idlen, t); //create x0 by BS2FQE
//
//         bls509_fp_sqr(t0, x0);    // t0 = x0^3 + b
//         bls509_fp_mul(t0, t0, x0);
//         bls509_fp_add(t0, t0, curve(z)->b);
//
//         if (bls509_fp_is_zero(t0))
//         {
//             ec_bls509_fp_point_set_xy(z, x0, t0);   //z = (x0, 0)
//             goto release;
//         }
//
//         mpz_add_ui(i, i, 1);   //i = i+1
//
//     } while (!bls509_fp_sqrt(y0, t0));
//
//     bls509_fp_set(y1, y0);   //y1 = y0
//     bls509_fp_neg(y2, y0);   //y2 = -y0
//
//     (mpz_cmp(mpz_rep(y1), mpz_rep(y2)) > 0) ? bls509_fp_set(y0, y2) : bls509_fp_set(y0, y1);
//
...
//-------------------------------------------
void ec_bls509_fp_to_oct(unsigned char *os, size_t *size, const EC_POINT P)
{
    size_t sx, sy;
    ...
    bls509_fp_to_oct(ox, &sx, xcoord(P));
    bls509_fp_to_oct(oy, &sy, ycoord(P));
...
void ec_bls509_fp_from_oct(EC_POINT z, const unsigned char *os, size_t size)
{
    ...
    case 0x00:
        ...
        break;
    case 0x04:
        bls509_fp_from_oct(z->x, &(os[1]), 64);
        bls509_fp_from_oct(z->y, &(os[65]), 64);
        bls509_fp_set_one(z->z);
        z->isinfinity = FALSE;
        break;
    }
}
