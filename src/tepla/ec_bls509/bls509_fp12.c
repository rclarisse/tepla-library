...
void bls509_fp12_inv(Element z, const Element x)
{
    ...
    bls509_fp6_sqr(t[0], rep0(x));  // t0 = a0^2
    bls509_fp6_sqr(t[1], rep1(x));  // t1 = a1^2

    bls509_fp6_gm_mul(t[2], t[1]);    //
    bls509_fp6_sub(t[0], t[0], t[2]); // t0 = t0 - t1*gamma
    bls509_fp6_inv(t[1], t[0]);       // t1 = t0^-1

    bls509_fp6_mul(rep0(z), rep0(x), t[1]); // c0 = a0*t1
    bls509_fp6_mul(rep1(z), rep1(x), t[1]); // c1 = a1*t1
    bls509_fp6_neg(rep1(z), rep1(z));       // c1 = -c1
}

void bls509_fp12_dob(Element z, const Element x)
{
    bls509_fp6_dob(rep0(z), rep0(x));
    bls509_fp6_dob(rep1(z), rep1(x));
}

void bls509_fp12_tri(Element z, const Element x)
{
    bls509_fp6_tri(rep0(z), rep0(x));
    bls509_fp6_tri(rep1(z), rep1(x));
}
...
void bls509_fp12_sqr(Element z, const Element x)
{
    ...
    {
        bls509_fp6_sub(t[0], rep0(x), rep1(x));     // t0 = a0 - a1
        bls509_fp6_gm_mul(t[1], rep1(x));           //
        bls509_fp6_sub(t[1], rep0(x), t[1]);        // t1 = a0 - gamma*a1
        bls509_fp6_mul(rep0(z), rep0(x), rep1(x));  // c0 = a0*a1
        bls509_fp6_mul(t[0], t[0], t[1]);           //
        bls509_fp6_add(t[0], t[0], rep0(z));        // t0 = t0*t1 + c0
        bls509_fp6_add(rep1(z), rep0(z), rep0(z));  // c1 = 2*a0*a1
        bls509_fp6_gm_mul(t[1], rep0(z));    // t1 = gamma*t1
        bls509_fp6_add(rep0(z), t[0], t[1]); // c0 = t1 + t0
    }

    if (strcmp(x->field->field_name, "bls509_fp12b") == 0)
    {
        bls509_fp6_add(t[0], rep0(x), rep1(x));
        bls509_fp6_gm_mul(t[1], rep1(x));
        bls509_fp6_add(t[1], t[1], rep0(x));
        bls509_fp6_muln(t[3], rep0(x), rep1(x));
        bls509_fp6_mod(rep1(z), t[3]);
        bls509_fp6_muln(t[4], t[0], t[1]);
        bls509_fp6_mod(t[0], t[4]);
        bls509_fp6_gm_mul(t[1], rep1(z));
        bls509_fp6_add(t[1], t[1], rep1(z));
        bls509_fp6_sub(rep0(z), t[0], t[1]);
        bls509_fp6_dob(rep1(z), rep1(z));
    }
}
...
{
    if (bls509_fp6_is_zero(rep1(x)))
    {
        if (bls509_fp6_is_zero(rep0(x))) {
            return TRUE;
        }
    }
    return FALSE;
...
{
    if (bls509_fp6_cmp(rep1(x), rep1(y)) == 0)
    {
        if (bls509_fp6_cmp(rep0(x), rep0(y)) == 0) {
            return 0;
        }
    }
    return 1;
...
int bls509_fp12_is_sqr(const Element x)
{
    ...
    bls509_fp6_inv(t[0], rep1(x));
    bls509_fp6_mul(t[0], t[0], rep0(x));
    bls509_fp6_sqr(t[0], t[0]);
    bls509_fp6_add(t[0], t[0], field(x)->irre_poly[0]);

    hr = bls509_fp6_is_sqr(t[0]);
...
{
    bls509_fp6_random(rep0(z));
    bls509_fp6_random(rep1(z));
}
